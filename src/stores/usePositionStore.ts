// src/stores/usePositionsStore.ts
...
export const usePositionsStore = create<PositionsStore>((set) => ({
  // ✅ SAFE DEFAULT
  positions: Array.isArray(initialPositions) ? initialPositions : [],
  comparePositions: [],

  addPosition: (newPos) =>
    set((state) => {
      const idx = state.positions.findIndex(
        (p) => p.symbol === newPos.symbol
      );
      ...
      if (idx !== -1) {
        ...
        const weightedAvg =
          (existing.qty * existing.avgPrice +
            newPos.qty * newPos.avgPrice) / totalQty;
      ...
      return {
        positions: [
          ...state.positions,
          { ...newPos, id: `pos-${Date.now()}` } as Position,
        ],
      };
    }),
  ...
  removePosition: (id) =>
    set((state) => ({
      ...
      comparePositions: state.comparePositions.filter(
        (p) => p.id !== id
      ),
    })),

  togglePositionCompare: (pos) =>
    set((state) => {
      const exists = state.comparePositions.some(
        (p) => p.id === pos.id
      );
      ...
      return {
        comparePositions: [...state.comparePositions, pos],
      };
    }),
